...
- Run Bash synchronously with `timeout_seconds` (default 300s). Retry with a higher timeout on timeout. For >10 min commands, run in the background, redirect output to a file, and poll periodically.
...
This mechanism allows the agent to accumulate project knowledge over time without requiring the user to repeat themselves.  The conflict-resolution clause mandates that the agent actively resolve contradictions when updating the file.  The prohibition on code snippets and symbols keeps the file compact and robust against code changes; the one-off task exclusion prevents preference drift due to idiosyncratic details.
...
1. After 3 failed retries of the same fix, rethink from scratch.
\end{prompt}
