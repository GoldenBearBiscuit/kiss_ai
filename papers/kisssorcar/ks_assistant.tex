...
\medskip
\noindent These two sentences occupy only two lines, yet they establish a behavioral contract that permeates every subsequent interaction.  We observe that removing any single directive degrades the agent's behavior along the corresponding dimension during KS Assistant development (e.g., removing ``BE RELENTLESS'' causes the agent to give up after the first error more frequently).
...
\noindent The rationale is a two-phase separation between \emph{collection} and \emph{synthesis}.  LLMs tend to anchor on the first few results they encounter, biasing their solutions toward a narrow slice of the design space.  By forcing the agent to accumulate a broad set of information into a file \emph{before} reasoning about it, the protocol counteracts anchoring bias and encourages the model to consider diverse approaches. The ``visit $\geq$30 websites'' threshold is deliberately aggressive: it ensures the agent does not shortcut the research phase after two or three hits, and the resulting information file serves as an auditable artifact of what the agent considered.  
...
%----------------------------------------------------------------------
\section{Painless software engineering with KS Assistant}
\label{sec:painless}
%----------------------------------------------------------------------
...
We skip the description of the remaining 2 tasks in the paper for space reasons, but they can be found in the Appendix.
...
\textbf{Code generation agents.}  SWE-Agent~\citep{yang2024sweagent} and OpenHands~\citep{wang2024openhands} provide LLM-based agents for resolving software engineering tasks.  Agentless~\citep{xia2024agentless} shows that a two-phase localize-then-repair pipeline can achieve competitive results.  Devin~\citep{devin2024}, Claude Code~\citep{claudecode2025}, Codex~\citep{codex2025}, and Aider~\citep{aider2023} offer various approaches to autonomous coding.  Cursor released Composer~2~\citep{cursor2026composer2}, a fine-tuned coding model trained with large-scale reinforcement learning.  

\textbf{Reasoning, planning, and multi-agent systems.}  ReAct~\citep{yao2023react}, chain-of-thought prompting~\citep{wei2022chain}, Tree of Thoughts~\citep{yao2023tree}, and Reflexion~\citep{shinn2023reflexion} structure step-by-step reasoning, search, and verbal self-critique; our continuation protocol relates to Reflexion but uses summaries to continue tasks rather than retry them.  ChatDev~\citep{qian2024chatdev}, MetaGPT~\citep{hong2024metagpt}, and AutoGen~\citep{wu2023autogen} model software development as multi-agent conversations; we use a single agent with broad tool access and optional parallel sub-agents.
...
\textbf{Benchmarks, agentic SE, and self-improvement.}  SWE-bench~\citep{jimenez2024swebench}, HumanEval~\citep{chen2021evaluating}, LiveCodeBench~\citep{jain2024livecodebench}, and SWE-bench Pro~\citep{deng2025swebenchpro} evaluate coding agents at various scales; \citet{prathifkumar2025swebenchverified} raises concerns about benchmark contamination.  Recent surveys and studies articulate the pillars of agentic software engineering, autonomous coding agents, agentic programming, agent context files, and context engineering~\citep{hassan2025agentic,li2025rise,wang2025agentic,chatlatanagulchai2025readmes,mohsenimofidi2025context}; our system prompt and per-repository override mechanisms instantiate these context-engineering patterns.  \citet{robeyns2025selfimproving} demonstrates a self-improving coding agent; our self-improvement loop is a lighter-weight form based on accumulated preferences.  \citet{gao2025trae} applies test-time compute scaling to software engineering, and Get Shit Done~\citep{gsd2025} is a meta-prompting system for coding agents that addresses context rot through phased planning documents, which inspired parts of our system prompt.
...
A simple, layered, single-concern architecture, combined with a system prompt that encodes engineering discipline, addresses the practical challenges of deploying LLM agents for real-world software development.  On Terminal-Bench~2.0, our system reaches 62.2\% (277/445), outperforming Claude~Code (58\%) and Cursor Composer~2 (61.7\%) on the same benchmark and base model~\citep{anthropic2026opus46} without benchmark-specific tuning, fine-tuning, or reinforcement learning.  Time-tested software engineering principles, expressed as concrete instructions an LLM can follow, matter more than model-level customization for building reliable, practical agent systems.  By building KS Assistant using itself and matching or beating both the Cursor and Claude Code agents, we demonstrate that time-tested software engineering principles and techniques are invaluable for building reliable, practical agent systems.

\textbf{Limitations.}  Our evaluation uses a single model and benchmark; broader evaluation, especially on benchmarks like SWE-bench Pro, is future work.  Continuation summaries may lose fidelity, and our quality-over-speed posture may not suit latency-sensitive workflows.
...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract and introduction (Section~\ref{sec:intro}) clearly state the contributions---a five-layer agent architecture, a structured system prompt, a VS~Code extension, and a 62.2\% pass rate on Terminal-Bench~2.0---and the scope is limited to software engineering tasks.  The conclusion's claim that KS~Assistant matches or beats both the Cursor Composer~2 and Claude~Code agents is directly supported by the head-to-head Terminal-Bench~2.0 results in Section~\ref{sec:evaluation} (62.2\% vs.\ 61.7\% and 58\%, on the same benchmark and base model), and the claim that KS~Assistant was built using itself is documented in Section~\ref{sec:intro} and discussed throughout Sections~\ref{sec:architecture}--\ref{sec:evaluation}.
    \item[] Guidelines:
    \begin{itemize}
