...
The system prompt is a structured document that governs the agent's behavior across all tasks.  It is not a generic instruction to ``be helpful'' but rather a precise specification of an engineering discipline.
...
\textbf{Calm, direct tone.}  An earlier version of this prompt used aggressive capitalized directives (e.g., ``BE RELENTLESS,'' ``NO AI SLOP'').  We found through extensive use that frontier models (Claude Opus~4.7, GPT~5.5, Gemini~3.1) respond more reliably to calm, direct language.  Anthropic's official prompting guide notes: ``Where you might have said `CRITICAL: You MUST use this tool when\ldots', you can use more normal prompting''~\citep{anthropic2025prompting}.  Aggressive emphasis causes overtriggering and unpredictable prioritization in newer models.  We replaced all such directives with positive, explanatory framing.
...
- If running out of context or steps, do not rush. Call
  ...
  in a new context.
- At step 98 of 100: you must call
  finish(is_continue=True) with a chronological summary
  of work done, so the task can be resumed.
</tool_rules>
\end{prompt}
...
\textbf{Context and continuation.}  When the context window is nearly full, LLMs exhibit a ``rush to finish'' behavior: they skip verification steps, make hasty edits, and call \texttt{finish} with an incomplete result.  The continuation instructions redirect that urgency into the continuation protocol (Section~\ref{sec:relentless-agent}), ensuring that a clean handoff to a new sub-session produces better results than a frantic attempt to squeeze everything into the remaining tokens.  The explicit ``step 98 of 100'' threshold provides a concrete trigger point that the model can check mechanically, rather than requiring it to estimate how much context remains.
...
sufficient context, proceed directly without web
research.
\end{prompt}
...
We have shown that a simple layered, single-concern architecture can address the practical challenges of deploying LLM agents for real-world software development.  On Terminal Bench~2.0, a benchmark of 89 diverse terminal-based tasks evaluated across 5 trials each, our system achieves a 62.2\% overall pass rate (277/445)---outperforming Claude~Code (58\%) and Cursor Composer~2 (61.7\%) on the same benchmark with the same underlying model (Claude Opus~4.6).  These results are achieved without benchmark-specific optimizations, fine-tuning, or reinforcement learning.
